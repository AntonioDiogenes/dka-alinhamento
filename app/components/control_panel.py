...
3. Status dos Sensores DKA (TCP 5000) e Habilitar Edição Manual.
4. Botoeira de Navegação FIXA NO RODAPÉ (Trocar Eixo, Configuração, Avançar ->).
"""
...
class ControlPanel(tk.Frame):
    ...
    def _build_ui(self):
        # =========================================================================
        # 1. BOTOEIRA DE NAVEGAÇÃO INFERIOR (PINADA NO RODAPÉ DO PAINEL)
        # Ao empacotar side="bottom" PRIMEIRO, garantimos que a botoeira 
        # (Trocar Eixo, Configuração e Avançar) fique FIXA no rodapé e NUNCA seja cortada!
        # =========================================================================
        bot_nav = tk.Frame(self, bg="#272f43")
        bot_nav.pack(fill="x", side="bottom", pady=(10, 0))
        ...
        self.btn_switch_axle.pack(fill="x", pady=(0, 8))

        # Linha com Botão Configuração (<-) e Avançar (->)
        ...
        # =========================================================================
        # 2. ÁREA SUPERIOR ROLÁVEL (Aro, Eixo Ativo, Cabeçotes DKA)
        # O Canvas rolável preenche todo o espaço vertical restante acima dos botões fixos.
        # =========================================================================
        self.scroll_canvas = tk.Canvas(self, bg="#272f43", highlightthickness=0, bd=0)
        self.scroll_canvas.pack(fill="both", expand=True, side="top")

        self.scroll_inner = tk.Frame(self.scroll_canvas, bg="#272f43")
        ...
        # --- CONTROLES INTERNOS NA ÁREA ROLÁVEL ---

        # 2.1. AJUSTE DE ARO DA RODA (13" a 30")
        rim_box = tk.Frame(self.scroll_inner, bg="#1a1f2e", highlightbackground="#2a3245", highlightthickness=1, padx=12, pady=10)
        rim_box.pack(fill="x", pady=(0, 10))
        ...
        # 2.2. IDENTIFICADOR DE EIXO ATIVO
        axle_box = tk.Frame(self.scroll_inner, bg="#1a1f2e", highlightbackground="#2563eb", highlightthickness=1, padx=12, pady=12)
        axle_box.pack(fill="x", pady=(0, 10))
        ...
        self.lbl_axle_name.pack(anchor="w", pady=(3, 0))

        # 2.3. STATUS DOS SENSORES DKA (TCP 5000)
        sensor_box = tk.Frame(self.scroll_inner, bg="#1a1f2e", highlightbackground="#334155", highlightthickness=1, padx=10, pady=10)
        sensor_box.pack(fill="x", pady=(0, 8))
        ...
        for pos_id, name, r, c in heads_layout:
            cell = tk.Frame(grid_s, bg="#111520", padx=5, pady=3, highlightbackground="#2a3245", highlightthickness=1)
            cell.grid(row=r, column=c, padx=2, pady=2, sticky="ew")
            grid_s.columnconfigure(c, weight=1)

            lbl_led = tk.Label(cell, text="●", font=("Segoe UI", 9, "bold"), fg="#6b7280", bg="#111520")
            lbl_led.pack(side="left")

            lbl_name = tk.Label(cell, text=name, font=("Segoe UI", 8, "bold"), fg="#ffffff", bg="#111520")
        ...
        self.btn_toggle_override.pack(fill="x", pady=(8, 0))
